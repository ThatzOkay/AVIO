...
import { chmodSync, existsSync, mkdirSync, realpathSync, rmSync, symlinkSync } from 'node:fs'
...
console.log('→ Forcing pinned wlroots-0.20 subproject (carries the AVIO output-control patch)')
...
const isSystemExcluded = (p: string): boolean => SYSTEM_EXCLUDED.some((re) => re.test(p))
...
while (idx < pendingLibs.length) {
  ...
  if (linkName !== realBase && !existsSync(destLink)) symlinkSync(realBase, destLink)

...
_DROPIN_CFG        = os.path.join(_DROPIN_DIR, "avio-no-sap.conf")
    ...
    if os.path.isdir(_DROPIN_DIR):
        try:
            for entry in os.listdir(_DROPIN_DIR):
                if not entry.startswith("avio-") or not entry.endswith(".conf"):
                    continue
...
AVIO_PLAYER_PATH   = "/avio/bt/player"
...
class AvioMediaPlayer(dbus.service.Object):
...
def main():
    ...
    wireless_mode = os.environ.get('AVIO_MODE', 'wireless') == 'wireless'
    ...
    def _atexit_cleanup():
        ...
        if adm is not None:
            try: adm.UnregisterAdvertisement("/avio/bt/ble")
            except Exception: pass
        pm = _bluez_handles["profile_manager"]
        if pm is not None:
            for path in ("/avio/bt/aa", "/avio/bt/hfp", "/avio/bt/hsp_hs"):
    ...
    if wireless_mode:
        aa_obj = AAProfile(bus, "/avio/bt/aa")
        ...
        try:
            profile_manager.UnregisterProfile("/avio/bt/aa")
    ...
    if wireless_mode:
        ble_ad_obj = BLEAd(bus, "/avio/bt/ble")
        ...
        try:
            ...
            if ad_manager_path:
                ...
                try:
                    ad_manager.UnregisterAdvertisement("/avio/bt/ble")
    ...
    # HFP Hands-Free profile — car kit role (AVIO = HF, Phone = AG)
    try:
        profile_manager.UnregisterProfile("/avio/bt/hfp")
        dprint("[aa-bt] unregistered stale HFP profile (previous run)")
    except Exception:
        pass

    global _hfp_profile_registered
    hfp_obj = HFPProfile(bus, "/avio/bt/hfp")
    try:
    ...
    try:
        profile_manager.UnregisterProfile("/avio/bt/hsp_hs")
    except Exception:
        pass
    try:
        hsp_obj = DummyProfile(bus, "/avio/bt/hsp_hs")
    ...
    agent = PairingAgent(bus, "/avio/bt/agent")
    ...
    # ── BlueZ media player (AVRCP target) ───────────────────────────────────
    try:
        avio_player = AvioMediaPlayer(bus, AVIO_PLAYER_PATH)
        media_iface = dbus.Interface(
            bus.get_object(BLUEZ_SERVICE, ADAPTER_PATH), MEDIA_IFACE
        )
        media_iface.RegisterPlayer(AVIO_PLAYER_PATH, _MPRIS_PLAYER_PROPS)
        _bluez_handles["avio_player"] = avio_player
        _bluez_handles["media_iface"] = media_iface
        dprint(f"[aa-bt] media player registered at {AVIO_PLAYER_PATH}", flush=True)
    except Exception as e:
    ...
    print()
    print("=== AVIO Android Auto stack running ===")
    print(f"  WiFi : SSID={SSID!r}  IP={AP_IP}  port={AA_PORT}")
    print(f"  BT   : {BTNAME}  (discoverable)")
    print()
    print("[aa-bt] Waiting for phone — TCP server is hosted in-process by AVIO")
    print()
    ...
    try:
        ...
        _set_advertising(False)
        for path in ("/avio/bt/aa", "/avio/bt/hfp"):
            try:

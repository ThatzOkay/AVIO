...
/// Spawns the gst-video pipeline in the standalone `gst-host` binary (not the app's own process,
/// which would export a bundled libffi that can corrupt wayland marshalling) and forwards calls
/// over a unix socket. Linux only; a no-op elsewhere since `gst_video::GstVideo` renders
/// in-process on mac/Windows.
#[derive(Default)]
pub struct GstHost {
...
impl GstHost {
    ...
    pub async fn push_buffer(handle: &GstHostHandle, app: &tauri::AppHandle, id: u32, nal: &[u8]) {

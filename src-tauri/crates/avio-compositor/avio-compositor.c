/*
 * avio-compositor — a nested wlroots compositor for AVIO (based on tinywl, 0.20).
 *
 * One screen per role (AVIO_SCREENS), each a nested output with a transparent Electron
...
#define AVIO_SCREEN_X_SLOT 100000

#define AVIO_TITLEBAR_H 32
#define AVIO_BTN_W 32
#define AVIO_BTN_GAP 2
#define AVIO_RESIZE_BORDER 8
...
struct tinywl_server {
	...
	struct avio_video_cfg video_cfgs[AVIO_MAX_VIDEO_CFGS];
	int ctrl_fd;
...
static void server_new_input(struct wl_listener *listener, void *data) {
	...
	case WLR_INPUT_DEVICE_TOUCH:
		/* AVIO: the head unit is a touchscreen, route touch through the cursor */
		wlr_cursor_attach_input_device(server->cursor, device);
...
static const struct wlr_buffer_impl avio_deco_buffer_impl = {
	.destroy = avio_deco_buffer_destroy,
	.begin_data_ptr_access = avio_deco_buffer_begin_data_ptr_access,
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	if (s->title != NULL)
		avio_scene_set_cairo(s->title, avio_draw_title(role_title(s->role), AVIO_TITLEBAR_H));
	if (s->btn_min != NULL)
		avio_scene_set_cairo(s->btn_min, avio_draw_button(AVIO_SYM_MIN, AVIO_BTN_W, AVIO_TITLEBAR_H));
	if (s->btn_fs != NULL)
		avio_scene_set_cairo(s->btn_fs, avio_draw_button(AVIO_SYM_FS, AVIO_BTN_W, AVIO_TITLEBAR_H));
	if (s->btn_close != NULL)
		avio_scene_set_cairo(s->btn_close, avio_draw_button(AVIO_SYM_CLOSE, AVIO_BTN_W, AVIO_TITLEBAR_H));
...
static void ctrl_init(struct tinywl_server *server) {
	...
	server->ctrl_fd = fd;
	wlr_log(WLR_INFO, "avio: control socket at %s", path);
}
